...
export const monthlyRepayableExpenses = (expenses) => {
  return Math.round(
    expenses
...
export const monthlyTaxableRepayableExpenses = (expenses) => {
  return Math.round(
    expenses
      .filter((expense) => expense.repayable && expense.taxable)
      .reduce((total, expense) => total + expense.monthlyAmount, 0)
...
export const monthlyExpenses = (expenses) => {
  return Math.round(
    expenses
...
export default function Expenses() {
  ...
  useEffect(() => {
    ...
    if (expenses) {
      setExpenses(JSON.parse(expenses).map((expense) => new Expense(expense)));
    } else {
